...
产品必须把以下九步放在同一项目中，而不是拆成互不相连的工具：

1. 说出想法：创建 TopicBrief，记录研究对象、目标、范围和未知项；
2. 查看已有研究：执行多源检索、去重、论文卡、流派、共识、争议与反向查询；
3. 选择课题：比较 2-3 张候选课题卡，由人完成 G0；
4. 理论与设计：形成理论机制、研究问题、识别/求解思路、假设与 G1；
5. 数据与合规：选择数据、变量、方法、公式，检查许可、隐私、测量和 G2；
6. 分析计划和代码：生成可编辑 AnalysisPlan/do-file，由人完成 G3；
7. 运行与稳健性：保存环境、日志、表图、诊断、复现信息和失败状态；
8. 证据与解释：把 Claim 连接到论文、数据、结果和反证，由人完成 G4；
9. 写作与交付：从已确认对象生成报告、HTML、引用和研究包，由人完成 G5。

每一步均提供当前任务、AI 草稿、结构化编辑、依据查看、批准/退回和下一步。六天比赛版本采用单用户本地实现，但九步均须有真实代码路径。
...
- 九步 Web 工作台、ProjectStore 和阶段状态机；
...
组织级 PostgreSQL、对象存储、任务队列、OIDC/RBAC、多用户协作和生产级 Runner 隔离属于部署增强，不应阻塞九步单用户产品闭环。
