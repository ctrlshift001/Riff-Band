...
class GenericMainPromptBuilder:
    ...
    def _format_subagent_capabilities(tools: List[Any] | None) -> str:
        ...
        if not names:
            return "SubAgent 会获得标准执行工具集，可进行资料读取、联网搜索、记录发现、写交接笔记、撰写产物和验证。"
    ...
    def build_prompt(
        ...
        profile_name = str(meta.get("profile_name", "generic"))
        task_goal = str(meta.get("task_goal", "完成用户任务。"))
        ...
        model_list = ", ".join(sub_models) if sub_models else "默认模型"
        generic_goals = {
            "Complete the assigned task and produce a verifiable deliverable.",
            "完成用户任务并交付可验证结果。",
        }
        target_lines: List[str] = []
        if task_goal and task_goal not in generic_goals:
            target_lines.append(f"- {task_goal}")
        if profile_name and profile_name != "generic":
            target_lines.append(f"- profile: {profile_name}")
        target_text = "\n".join(target_lines) if target_lines else "- 按用户任务本身确定交付目标。"
        final_decision_text = (
...
- 剩余轮次: {max_attempts - attempt_index}
- 可用 SubAgent 模型: {model_list}
...
SubAgent 能力：
{GenericMainPromptBuilder._format_subagent_capabilities(tools)}
...
- 只返回 JSON。
- 每轮只能选择一个 action。
- 可省略 `model` 和 `tools`；系统会自动补默认值。

Action schema：
- delegate_task: {{"task_instruction": "任务类型: research|write|verify|continue\\n期望产出: ...\\n完成标准: ...\\n具体任务: ...", "context": "...", "model": "可选", "tools": "可选"}}
- delegate_tasks: {{"max_concurrency": {max_parallel}, "tasks": [{{"task_instruction": "任务类型: research|write|verify|continue\\n期望产出: ...\\n完成标准: ...\\n具体任务: ...", "context": "...", "model": "可选"}}]}}
- continue_task: {{"session_id": "...", "task_instruction": "任务类型: continue\\n期望产出: ...\\n完成标准: ...\\n具体任务: ...", "context": "...", "model": "可选"}}
- wait_worker_sessions: {{"session_ids": ["..."], "timeout_seconds": 30}}
- inspect_worker_session: {{"session_id": "...", "include_trace": true}}
- list_worker_sessions: {{"include_closed": false}}
- close_worker_session: {{"session_id": "...", "reason": "..."}}
- complete_task: {{"executive_summary": "...", "status": "done|partial|blocked", "artifacts": [{{"type": "report|file|data|note", "path": "...", "description": "..."}}], "verification": ["..."], "open_issues": [], "confidence": "high|medium|low", "report_path": "{report_path}", "findings_path": "{findings_path}", "required_sections": {json.dumps(required_sections, ensure_ascii=False)}, "min_findings": {min_findings}}}
...
- write: 目标是产出指定内容。优先读取已有 findings/scratchpad/source，只写指定章节或文件，不做广泛搜索；写完后 `finish`。
