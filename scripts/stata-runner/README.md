...
1. 在 PowerShell 或命令提示符中运行：
   ...
   ```bat
   SETUP_STATA_RUNNER.bat -StataExecutable "C:\Program Files\Stata19\StataMP-64.exe" -DockerWorkbench
   ```

   该开关会让 Runner 监听 `0.0.0.0`，并把相同随机令牌同步到本机
   `deployment/competition/.env.competition`。端口仍必须由系统防火墙限制为仅本机 Docker 使用。
...
Runner 默认只监听 `127.0.0.1:8765`。通过 Docker 访问时必须使用 `-DockerWorkbench`，并按 `docs/RUNNER.md` 检查 `host.docker.internal` 和主机防火墙。
